...
export default function UpcomingJobs() {
  ...
  async function completeBooking(id, bookingRequest, index) {
    ...
    setProcessing(true);
    try {
      const fetchedbookings = await axios.put("http://localhost:3001/request/completeBooking/" + id, bookingRequest); 
      if (fetchedbookings.data) {
    ...
    RequestList = (
      bookings.map((booking, index) => {
        return <Booking key={index} 
          booking={booking} 
          complete={ () => completeBooking(booking.request._id, booking.request, index) } 
        />
      })
    )
  }
  ...
  return (
      <>
        ...
        {error && <Alert severity="error">Error! Cannot complete Task at this time!</Alert>}
        {success && (
